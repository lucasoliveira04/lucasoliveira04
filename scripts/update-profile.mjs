...
const titles = {retro:'HIGH SCORES / GITHUB',futurama:'TELEMETRIA DA MISSÃO','windows-xp':'Propriedades de GitHub',intellij:'Git · Repository insights',minimalista:'O trabalho, em números.'};
for (const theme of config.themes) {
  ...
  for (const [x,value,label] of columns) body += text(x,116,49,p.fg,value,'font-weight="700"') + text(x,148,20,p.muted,label);
...
await activate();
console.log(`Atualizados 5 temas e README principal: ${data.publicOriginalRepos} repositórios próprios públicos.`);
